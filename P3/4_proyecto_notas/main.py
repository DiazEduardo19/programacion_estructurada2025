...
def menu_notas(usuario_id,nombre,apellidos):
    while True:
        ...
        if opcion == '1' or opcion=="CREAR":
            ...
            print(f"{'ID':<10}{'Titulo':<15}{'Descripción':<20}{'Fecha':<15}")
            print(f"-"*80)
            respuesta_borrar=input("Deseas borrar alguna nota (Si/No):").lower
            if respuesta_borrar == "si":
                    print(f"\n \t .:: {nombre} {apellidos}, vamos a borrar un Nota ::. \n")
                    id = input("\t \t ID de la nota a eliminar: ")
            #Agregar codigo
            respuesta=nota.borrar(id)
            if respuesta:
                (f"\n\t Se borro correctamente la nota {id} exitosamente")
            else:
                print(f"\n\t No fue posible borrar en este momento intentelo mas tarde")
            funciones.esperarTecla()    
